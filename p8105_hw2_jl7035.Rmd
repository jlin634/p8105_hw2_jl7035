...
```{r}
bakers_df <- 
  read_csv("Local_Data_Files/gbb_datasets/bakers.csv",
...
```{r}
bakes_df <- 
  read_csv("Local_Data_Files/gbb_datasets/bakes.csv",
  na= c("NA", ".", "", "N/A", "Unknown", "UNKNOWN")) %>% 
  janitor::clean_names() %>%
  relocate(baker)
...
anti_join(results_df, bakers_df)
```
...
anti_join(results_df, gbb_combined)
